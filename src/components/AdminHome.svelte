  ...
  <div class="relative z-10 flex flex-col min-h-screen">
    ...
    <main class="container mx-auto px-6 py-12 animate-fade-in-up animation-delay-700 flex-grow">
      <div class="flex justify-center">
        <div class="w-full max-w-2xl transform hover:scale-[1.02] transition-all duration-500 hover:drop-shadow-2xl">
          <AdminUpload />
        </div>
      </div>
    </main>
...
<style>
  ...
  .particle-2 { top: 60%; left: 80%; animation-delay: 2s; animation-duration: 6s; }
  .particle-3 { top: 80%; left: 20%; animation-delay: 4s; animation-duration: 10s; }
  .particle-4 { top: 30%; left: 70%; animation-delay: 1s; animation-duration: 7s; }
  .particle-5 { top: 50%; left: 30%; animation-delay: 3s; animation-duration: 9s; }
  .particle-6 { top: 10%; left: 60%; animation-delay: 5s; animation-duration: 8s; }
  ...
  @keyframes float {
    ...
    75% { transform: translateY(-20px) translateX(-10px) scale(1.1); opacity: 0.8; }
  }

  /* Wave animations */
  .wave-animation { position: absolute; bottom: 0; left: 0; width: 100%; height: 100%; animation: wave 4s ease-in-out infinite; }
  .wave-2 { animation: wave 6s ease-in-out infinite reverse; opacity: 0.7; }

  @keyframes wave { 0%, 100% { transform: translateX(0px); } 50% { transform: translateX(-20px); } }
  ...
  .hover\:drop-shadow-2xl:hover { filter: drop-shadow(0 25px 25px rgba(0, 0, 0, 0.15)); }

  /* Responsive adjustments */
  @media (max-width: 768px) {
    .particle { width: 3px; height: 3px; }
    .wave-animation { height: 60px; }
  }
</style>
