...
test('admin redirects to login when unauthenticated and login flow works', async ({ page }) => {
  ...
  await page.context().addCookies([
    { name: 'admin_auth', value: '1', domain: 'localhost', path: '/', httpOnly: true },
  ]);
  await page.goto('/admin');
  await expect(page.getByRole('heading', { name: 'Admin Song Upload' })).toBeVisible();
